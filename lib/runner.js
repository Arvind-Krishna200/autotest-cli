...
// const { login } = require('./auth');  // ← deprecated, replaced by capture
...
async function runTests(url, options = {}) {
  ...
  //   log.push({ type: 'login', pass: true, label: `Login → ${testUrl}`, status: 200, external: false });
  // }
    ...
    if (type === 'xhr' || type === 'fetch') {
      ...
      if (!seenAPIs.has(apiKey) || !pass) {
        ...
        log.push({
          type: 'api', pass,
          label: `[${method}] ${reqUrl}`,
          status: respStatus, external: isExternal
        });
      }
    }
