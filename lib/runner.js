...
// These texts will NEVER be clicked — protects real data
const BLOCKED_TEXTS = [
  // Destructive
...
async function guardExternalRedirect(page, baseDomain, testUrl) {
  ...
  if (isExternal) {
    ...
    await page.goto(testUrl, { waitUntil: 'domcontentloaded', timeout: 10000 }).catch(() => {});
    return true;
  }
  return false;
}
...
// ── Menu Collector — full <a> tag support ─────────────────
async function getVisibleMenuItems(page) {
  return await page.evaluate(() => {
    ...
    const selectors = [
      // 1. Semantic nav <a> — covers OpenAI docs, modern SPAs
      'nav a',
      'header nav a',
      '[role="navigation"] a',
      // 2. Sidebar <a>
      ...
      '[role="tab"]',
      '[role="treeitem"]',
      // 6. Button-based menus (enterprise style)
      '.sidebar button',
      'nav button',
      '[class*="nav"] button',
      '[class*="menu"] button',
...
// ── Safe Click — with full security checks ────────────────
async function safeClick(page, text) {

  // 1. Block dangerous texts before anything
  if (isDangerous(text)) {
  ...
  try {
    ...
    // 2. Inspect element before clicking
    ...
    // 4. Skip if onclick has confirm() — destructive action pattern
    if (tagInfo.hasConfirm) {
      console.log(chalk.yellow(`  ⚠  Blocked confirm-action → "${text}"`));
      return false;
    }

    // 5. Block bare form submit buttons outside nav context
...
async function runTests(url, options) {
  ...
  // ── SECURITY: Auto-dismiss all dialogs ───────────────
  // Prevents confirm/alert/prompt from blocking the browser
  ...
  // ── SECURITY: Close any new tabs immediately ──────────
  // Prevents context loss when links open new windows
  page.context().on('page', async newPage => {
    const newUrl = newPage.url();
    console.log(chalk.yellow(`  ⚠  New tab blocked: ${newUrl}`));
    await newPage.close().catch(() => {});
  });


  // ── SECURITY: Cancel any file downloads ──────────────
  // Prevents accidental downloads during nav testing
  ...
  page.on('response', async response => {
    ...
    if (type === 'xhr' || type === 'fetch') {
      ...
      apiResults.push({ url: reqUrl, status, method, pass, isExternal });
      log.push({
        type: 'api',
        pass,
        label: `[${method}] ${reqUrl}`,
  ...
  for (let i = 0; i < uniqueMainItems.length; i++) {
    ...
    try {
      ...
      // ── Guard external redirect after click ──────────
      ...
      for (const subItem of uniqueSubItems) {
        try {
          ...
          // ── Guard external redirect on sub click ────
